...
对于验证结果为 ⚠️ 或 ❌ 的条目：
- **数据不准确**：替换为可查证的数据，标注来源
- **引用无出处**：替换为嘉宾本人的可溯源原话，或直接删除
- **日期错误**：修正为经 `web_search` 验证的准确日期
- **不可验证的夸大性声明**：替换为保守但准确的描述

> **原则**：宁可使用表达力稍弱但准确的内容，也不使用有传播力但存疑的数据。信息图一旦发布传播，错误信息的修正成本极高。

#### 2.4 向用户报告筛查结果

将事实筛查结果呈现给用户，特别是 ⚠️ 修正项和 ❌ 删除项，让用户确认后再进入提示词编写阶段。
...
| 风格名称 | 英文 ID | 适用场景 |
|----------|---------|----------|
| **暖纸手账** | `warm-sketch` | 知识分享、方法论、产品分析（亲切温馨） |
| **黑金粉笔** | `blackboard-gold` | 技术架构、战略分析、行业趋势（权威高端） |

- 不指定时默认使用 `warm-sketch`
- 在 `gemini_charts_config.json` 中通过 `"style"` 字段切换
