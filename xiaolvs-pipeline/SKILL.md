...
- **确认点之间的执行阶段不要停顿（2026-09-06 用户明确要求）**：图片生成用 `--until-done` 主循环一次跑完全部（撞配额自动等待续跑），**不要每生成一张图就停下来等用户输入「继续」**。只有登录/扫码/授权等人工环节才停下来等确认，且不要后台轮询
- **图片生成通道**：用户首选 ChatGPT 网页 CDP（chatgpt.com/images，见 xiaolvs-image-gen 的通道路由），不要用内置 ImageGen API 代替
...
- **交付前逐张验证图片（2026-09-06 教训）**：不能只看文件存在。校验 PNG 真实格式、宽度 ≥900、>500KB、MD5 互不相同，并逐张查看内容确认主题正确。曾发生把 512x512 WebP 缩略图当成果交付的事故
